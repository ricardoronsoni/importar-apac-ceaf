...
CREATE TABLE IF NOT EXISTS geral.sigtap (
    numero_procedimento char(10) not null primary key,
    descricao_procedimento varchar(200) not null,
    registro_ativo bool default true not null,
    data_alteracao timestamp default (now() at time zone 'America/Sao_Paulo') not null
);
comment on table geral.sigtap is 'Tabela com os descritivos dos procedimentos do CEAF na SIGTAP.';
comment on column geral.sigtap.numero_procedimento is 'Numero do procedimento da SIGTAP.';
comment on column geral.sigtap.descricao_procedimento is 'Nome do procedimento da SIGTAP.';
comment on column geral.sigtap.registro_ativo is 'Informa se o registro esta ativo ou nao.';
comment on column geral.sigtap.data_alteracao is 'Data da ultima atualizacao no registro.';